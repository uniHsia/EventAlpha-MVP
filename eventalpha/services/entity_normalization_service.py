"""Entity normalization based on a small alias dictionary."""
...
class EntityNormalizationService:
    """Normalize extracted entities without mixing them with assets."""

    def __init__(
        self,
        alias_path: str | Path = "eventalpha/rules/entity_aliases.yaml",
    ) -> None:
    ...
    def standard_names(self) -> list[str]:
        """Return standard entity names in config order."""
        return list(self._standard_names)

    def normalize_entity_name(self, name: str) -> str:
        """Normalize one entity name, preserving unknown values."""
        ...
        if normalized:
            return normalized
        self.warnings.append(f"Unknown entity alias preserved: {raw_name}")
        return raw_name

    def normalize_entity_list(self, names: list[str]) -> list[str]:
        """Normalize and deduplicate entity names while preserving order."""
        self.warnings = []
        results: list[str] = []
        seen: set[str] = set()
        for name in names:
    ...
    def _load_aliases(self) -> None:
        if not self.alias_path.exists():
            raise FileNotFoundError(f"Entity alias file not found: {self.alias_path}")
        payload = yaml.safe_load(self.alias_path.read_text(encoding="utf-8")) or {}
        if not isinstance(payload, dict):
            raise ValueError(f"Entity alias file must be a mapping: {self.alias_path}")
